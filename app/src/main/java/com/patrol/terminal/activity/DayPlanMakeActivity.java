...
public class DayPlanMakeActivity extends AppCompatActivity {
    ...
    private void initview() {
        ...
        weekNum.setText(time);
        String[] years = time.split("年");
        String[] months = years[1].split("月");
        month = Integer.parseInt(months[0]) + "";
        year = years[0];
        if (months.length > 1) {
            week = months[1].substring(1, 2);
        }
        weekPlanName.setText(time+"计划");
        String typeName = SPUtil.getString(this, Constant.TYPE, Constant.TASKTYPENAME, "");
        weekPlanType.setText(typeName);
        getLine();
    }

    @OnClick({R.id.title_back, R.id.week_yes, R.id.week_add_line, R.id.week_delet_line, R.id.week_start_date})
...
//                .getDayLine(year, month, SPUtil.getTypeId(this))
            ...
            public void onOptionsSelect(int options1, int option2, int options3, View v) {
                for (int i = 0; i < results.size(); i++) {
                    if (lineList.get(options1).equals(results.get(i).getName())) {
                        WeekPlanView view = new WeekPlanView(DayPlanMakeActivity.this, results.get(i), year, month);
        ...
        pvTime = new TimePickerBuilder(this, new OnTimeSelectListener() {
            ...
            public void onTimeSelect(Date date, View v) {//选中事件回调
                ...
                if (type == 0) {
                    ...
                    if ("".equals(startTime)) {
                        Toast.makeText(DayPlanMakeActivity.this, "请先选择起始时间", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    end = date.getTime();
                    if (end < start) {
                        Toast.makeText(DayPlanMakeActivity.this, "结束时间不能低于起始时间", Toast.LENGTH_SHORT).show();
                        return;
                    }
