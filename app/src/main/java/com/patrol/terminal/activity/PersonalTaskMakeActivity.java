...
public class PersonalTaskMakeActivity extends AppCompatActivity {
    ...
    List<String> userAddList = new ArrayList<>();
    ...
    private void initview() {
        ...
        taskPersonalLl.setVisibility(View.VISIBLE);
        ...
        weekPlanName.setText(time+"个人任务");
    ...
    @OnClick({R.id.title_back, R.id.week_yes, R.id.week_add_line, R.id.week_delet_line, R.id.add_incontinuity_tower})
...
//        bean.set(weekPlanDetail.getText().toString().trim());
        ...
        bean.setUser_id(userAddList);
        BaseRequest.getInstance().getService()
                .savePersonalTask(bean)
    ...
    public void getLine() {
        BaseRequest.getInstance().getService()
                .getPersonalLine(year, month, SPUtil.getTypeId(this), dayNum)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    public void getperson() {
        BaseRequest.getInstance().getService()
                .getUserList(SPUtil.getDepId(this))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseObserver<List<UserBean>>(this) {
                    @Override
                    protected void onSuccees(BaseResult<List<UserBean>> t) throws Exception {
                        userBeanList = t.getResults();
                        if (userBeanList != null) {
                            for (int i = 0; i < userBeanList.size(); i++) {
                                userList.add(userBeanList.get(i).getName());
                            }
                        }
                    }
            ...
            public void onOptionsSelect(int options1, int option2, int options3, View v) {
                for (int i = 0; i < results.size(); i++) {
                    if (lineList.get(options1).equals(results.get(i).getName())) {
                        WeekPlanView view = new WeekPlanView(PersonalTaskMakeActivity.this, results.get(i), year, month);
    ...
    private void showPersonalLine() {// 不联动的多级选项
        OptionsPickerView pvOptions = new OptionsPickerBuilder(this, new OnOptionsSelectListener() {
            @Override
            public void onOptionsSelect(int options1, int option2, int options3, View v) {
                addTextView(userList.get(options1));
                userAddList.add(userBeanList.get(options1).getId());
    ...
    private void initEvents(final TextView tv) {
        tv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for (int i = 0; i < userBeanList.size(); i++) {
                    if (tower.equals(userBeanList.get(i).getName())) {
                        userList.add(i, tower);
                        userAddList.remove(i);
                    }
                }
        ...
        pvTime = new TimePickerBuilder(this, new OnTimeSelectListener() {
            ...
            public void onTimeSelect(Date date, View v) {//选中事件回调
                ...
                if (type == 0) {
                    ...
                    if ("".equals(startTime)) {
                        Toast.makeText(PersonalTaskMakeActivity.this, "请先选择起始时间", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    end = date.getTime();
                    if (end < start) {
                        Toast.makeText(PersonalTaskMakeActivity.this, "结束时间不能低于起始时间", Toast.LENGTH_SHORT).show();
                        return;
                    }
