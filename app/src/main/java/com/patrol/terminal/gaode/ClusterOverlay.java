package com.patrol.terminal.gaode;
...
//默认最多会缓存80张图片作为聚合显示元素图片,根据自己显示需求和app使用内存情况,可以修改数量
    ...
    private Cluster getCluster(LatLng latLng, List<Cluster> clusters) {
        for (Cluster cluster : clusters) {
            ...
            if (distance < mClusterDistance && mAMap.getCameraPosition().zoom < 20) {
                return cluster;
            }
        }
    ...
    private BitmapDescriptor getBitmapDes(int num) {
        ...
        if (bitmapDescriptor == null) {
            bitmapDescriptor = BitmapDescriptorFactory.fromView(mClusterRender.getView(num));
